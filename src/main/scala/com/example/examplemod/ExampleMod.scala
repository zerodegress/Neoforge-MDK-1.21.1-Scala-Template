...
class ExampleMod(val modEventBus: IEventBus, val modContainer: ModContainer):
  ...
  private def commonSetup(event: FMLCommonSetupEvent) = {
    ...
    if Config.logDirtBlock.get then
      LOGGER.info(
        "DIRT BLOCK >> {}",
  ...
  private def addCreative(event: BuildCreativeModeTabContentsEvent) =
    if event.getTabKey() == CreativeModeTabs.BUILDING_BLOCKS then
...
object ExampleMod:
  ...
  final val CREATIVE_MODE_TABS =
    DeferredRegister.create(Registries.CREATIVE_MODE_TAB, MODID)

  // Creates a new Block with the id "examplemod:example_block", combining the namespace and path
  final val EXAMPLE_BLOCK = BLOCKS.registerSimpleBlock(
    "example_block",
    BlockBehaviour.Properties.of().mapColor(MapColor.STONE)
  )
  // Creates a new BlockItem with the id "examplemod:example_block", combining the namespace and path
  final val EXAMPLE_BLOCK_ITEM =
    ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK)

  // Creates a new food item with the id "examplemod:example_id", nutrition 1 and saturation 2
  final val EXAMPLE_ITEM = ITEMS.registerSimpleItem(
    "example_item",
  ...
  final val EXAMPLE_TAB = CREATIVE_MODE_TABS.register(
    "example_tab",
    () =>
      CreativeModeTab
        .builder()
        .title(
          Component.translatable("itemGroup.examplemod")
